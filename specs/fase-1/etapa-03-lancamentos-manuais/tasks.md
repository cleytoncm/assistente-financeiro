...
- [x] T01 — Migration: colunas `is_active`, `is_hidden` em `accounts` e `cards`
      (requisito: RF-08)
- [x] T02 — Migration: tabela `categories` + seed com categorias comuns (receita e despesa)
      (requisito: RF-03)
- [x] T03 — Migration: tabela `transactions` (com `refund_of_transaction_id`,
      `installment_group_id`, `installment_number`, `installment_count`)
      (requisito: RF-01, RF-02, RF-06)
- [x] T04 — Endpoint POST /transactions (avulso) + validações + testes (criação válida, tipo
      inválido, conta/cartão informados juntos ou nenhum dos dois, conta de outro usuário
      rejeitada, conta desativada rejeitada)
      (requisito: RF-01)
- [x] T05 — Endpoint POST /transactions com `installments` + testes (divisão exata do total,
      arredondamento na última parcela, datas geradas mês a mês)
      (requisito: RF-02)
- [x] T06 — Validação de estorno (`refund_of_transaction_id`) + testes (tipo oposto, mesma
      conta/cartão, valor <= original aceito, valor maior rejeitado)
      (requisito: RF-06)
- [x] T07 — Endpoint GET /transactions (filtros + paginação) + teste
      (requisito: RF-07)
- [x] T08 — Endpoint PATCH /transactions/:id (+ `applyToRemaining`) + testes (edição simples,
      edição aplicada às parcelas restantes)
      (requisito: RF-04)
- [x] T09 — Endpoint DELETE /transactions/:id (+ `scope=single|remaining`) + teste
      (requisito: RF-05)
- [x] T10 — Endpoints POST /categories e GET /categories + testes (seed visível a todos,
      categoria própria isolada por usuário, tipo bate com o lançamento)
      (requisito: RF-03)
- [x] T11 — Estender GET /accounts e GET /cards com `date`/`includeHidden`,
      ...
      (requisito: RF-10)
- [x] T12 — Endpoints PATCH /accounts/:id/status e PATCH /cards/:id/status + teste
      (requisito: RF-08)
- [x] T13 — Estender DELETE /accounts/:id e DELETE /cards/:id com `cascade=true` + testes
      (bloqueio sem cascade quando há lançamentos, cascata remove tudo)
      (requisito: RF-09)
- [x] T14 — Formulário de lançamento (React): tipo, valor, data, descrição, conta/cartão,
...
- [x] T15 — Tela de extrato paginada com filtros + teste de componente
      (requisito: RF-07)
- [x] T16 — Ações de editar/remover lançamento com diálogo de escopo (parcela única vs.
      restantes) + teste de componente
      (requisito: RF-04, RF-05)
- [x] T17 — Seletor de data (padrão hoje) nas telas de contas/cartões, refletindo saldo/gasto +
      teste de componente
      (requisito: RF-10)
- [x] T18 — Toggles de ativar/desativar e mostrar/ocultar na listagem de contas/cartões + teste
      de componente
      (requisito: RF-08)
- [x] T19 — Modal de remoção com 3 opções quando há lançamentos associados + teste de
