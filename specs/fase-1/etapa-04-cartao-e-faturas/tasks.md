...
- [x] T01 — Migration: tabela `invoices`
      (requisito: RF-01, RF-02, RF-05)
- [x] T02 — Migration: coluna `invoice_id` em `transactions`
      (requisito: RF-01)
- [x] T03 — Lógica de resolução/criação sob demanda de `invoice_id` ao criar/editar
      ...
      (requisito: RF-01)
- [x] T04 — Cálculo de status derivado da fatura + testes unitários (todas as transições:
      aberta/fechada/atrasada/paga, incluindo o caso de `date` igual ao `closing_date`)
      (requisito: RF-03)
- [x] T05 — Endpoint GET /cards/:id/invoices + teste
      (requisito: RF-08)
- [x] T06 — Endpoint GET /invoices/:id e GET /invoices/:id/transactions + teste
      (requisito: RF-08)
- [x] T07 — Endpoint PATCH /invoices/:id (ajuste manual de datas) + teste (bloqueado quando já
      paga)
      (requisito: RF-02)
- [x] T08 — Endpoint POST /invoices/:id/pay + testes (pagamento normal, pagamento antecipado
      de fatura aberta, fatura já paga rejeitada)
      (requisito: RF-04, RF-05)
- [x] T09 — Trava de edição/remoção de Transaction em fatura não aberta + teste (409 esperado)
      (requisito: RF-07)
- [x] T10 — Fluxo de lançamento retroativo em fatura fechada/paga + ajuste do
      ...
      (requisito: RF-06)
- [x] T11 — Tela de fatura: listagem + detalhe com lançamentos e total + teste de componente +
      E2E (ver faturas do cartão, abrir detalhe)
      (requisito: RF-08)
- [x] T12 — Ação de editar fechamento/vencimento de uma fatura + teste de componente
      (requisito: RF-02)
- [x] T13 — Ação de pagar fatura com seleção de conta + teste de componente + E2E (pagar
      fatura, ver saldo da conta debitado)
      (requisito: RF-04, RF-05)
- [x] T14 — Modal de confirmação de ajuste de pagamento ao lançar em fatura já paga + teste de
      componente
      (requisito: RF-06)
- [x] T15 — Bloqueio visual de editar/remover lançamento de fatura não aberta + teste de
      componente
      (requisito: RF-07)
