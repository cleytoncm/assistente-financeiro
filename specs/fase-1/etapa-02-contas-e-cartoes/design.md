...
- `initial_balance` é o ponto de partida do saldo; saldo corrente será calculado somando
  lançamentos (Etapa 3), não armazenado como campo mutável aqui.
...
- `currency` é código ISO 4217 (texto livre, sem tabela de catálogo — lista de moedas é
  ...
  etapa (ver Fora de escopo do requirements.md).
...
DELETE /accounts/:id          remove conta (bloqueado se houver lançamentos — Etapa 3+)
...
DELETE /cards/:id             remove cartão (bloqueado se houver lançamentos — Etapa 3+)

POST   /banks                 cadastra banco no catálogo (name, code) — compartilhado, sem user_id
GET    /banks                 lista bancos do catálogo
```

Autenticação: esta etapa depende da Etapa 1 (Autenticação e Usuários). Todo endpoint exige o
...
entre usuários (RF-04 da Etapa 1) sem que esta etapa precise reimplementar nada de autenticação.
...
- Sem exclusão física por padrão seria uma opção (soft delete), mas como esta etapa (RF-04) só
  bloqueia remoção quando há lançamentos, exclusão física é suficiente por ora — revisitado na
  Etapa 3 (RF-09), que introduz exclusão em cascata como alternativa

## Frontend (React + Vite + TypeScript)
Reaproveita o client HTTP e a guarda de rota da Etapa 1.
